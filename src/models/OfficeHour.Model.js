...
export default class OfficeHour {
    ...
    readById (office) {
        let stmt =  "SELECT * FROM expediente WHERE cod_exp="+office.cod_exp+";"
        return this._queryExecutor.execute(stmt)
    }

    readByEmployee (office) {
        let stmt =  "SELECT * FROM expediente WHERE funcionarios_cod_func="+office.funcionarios_cod_func+";"
        return this._queryExecutor.execute(stmt)
    }
